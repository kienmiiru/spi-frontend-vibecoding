...
export default function PeraturanKeuangan() {
  ...
  const loadKriteria = async () => {
    setIsLoading(true);
    try {
      const data = await apiFetch("/api/keuangan/kriteria");
      setKriteriaText(data.value || "");
    } catch (err) {
      showNotification(
        err.message || "Gagal memuat kriteria audit default",
        "error"
      );
    } finally {
      setIsLoading(false);
  ...
  const handleSaveKriteria = async (e) => {
    ...
    setIsSaving(true);
    try {
      const updated = await apiFetch("/api/keuangan/kriteria", {
        method: "PUT",
      ...
      confirm({
        title: "",
        message: "Kriteria default audit Keuangan berhasil diperbarui",
        type: "info"
      });
    } catch (err) {
      showNotification(
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            Peraturan & Kriteria Keuangan
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Konfigurasi kriteria default untuk audit divisi Keuangan di seluruh Unit Kerja.
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSaveKriteria} className="space-y-6">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-2">
                Kriteria Default Audit Keuangan
              </label>
              <BulletedTextArea
                value={kriteriaText}
                onChange={setKriteriaText}
                placeholder="Tulis butir-butir peraturan/kriteria hukum default yang berlaku untuk audit keuangan..."
              />
