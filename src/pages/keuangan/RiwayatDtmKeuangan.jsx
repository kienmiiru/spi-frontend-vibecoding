...
import { Eye, Loader2, FileDown, Wallet } from "lucide-react";
...
export default function RiwayatDtmKeuangan() {
  ...
  const handleExportDocx = async (dtm) => {
    ...
    try {
      ...
      const blob = await apiFetchBlob("/api/keuangan/office/export-docx/setelah", {
        method: "POST",
        body: JSON.stringify({ dtmKeuanganId: dtm.id }),
      });

      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `DTM_Keuangan_${unitKerjaName}_${selectedYear}.docx`;
  ...
  const handlePreviewDocx = async (dtm) => {
    ...
    try {
      const unitKerjaName = getFacultyName(dtm.fakultasId);
      setPreviewTitle(`Pratinjau DTM Keuangan - ${unitKerjaName}`);
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            Riwayat DTM Keuangan
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Menampilkan semua DTM Keuangan yang telah ditandai selesai (diarsipkan).
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <Table>
            <Table.Header>
              <Table.Row>
                ...
                <Table.HeaderCell className="text-center w-40 font-mono">Tahun Anggaran</Table.HeaderCell>
          ...
          <div className="border border-gray-300 p-16 flex flex-col items-center justify-center text-center bg-gray-50/50">
            ...
            <span className="text-xs text-gray-400 font-semibold">
              Tidak ada riwayat DTM Keuangan yang sudah ditandai selesai untuk tahun anggaran {selectedYear}.
            </span>
          </div>
        )}
      </div>
