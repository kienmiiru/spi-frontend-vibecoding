...
import { Eye, Loader2, FileText, FileDown } from "lucide-react";

import YearDropdown from "../../components/YearDropdown";
import Notification from "../../components/Notification";
import { apiFetch, apiFetchBlob } from "../../lib/api";

export default function RiwayatDtmSdm() {
  ...
  const handleExportDocx = async (dtm) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `DTM_SDM_${unitKerjaName}_${selectedYear}.docx`;
  ...
  const handlePreviewDocx = async (dtm) => {
    ...
    try {
      const unitKerjaName = getFacultyName(dtm.fakultasId);
      setPreviewTitle(`Pratinjau DTM SDM - ${unitKerjaName}`);
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Menampilkan semua DTM SDM yang telah ditandai selesai.
          </p>
        </div>
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-300 text-xs font-semibold text-gray-600">
                  <th className="px-6 py-3 border-r border-gray-200 w-16 text-center">
                    No
                  </th>
                  <th className="px-6 py-3 border-r border-gray-200">Unit Kerja</th>
                  <th className="px-6 py-3 border-r border-gray-200 text-center w-40">
                    Tahun Anggaran
                  </th>
          ...
          <div className="border border-gray-150 rounded-lg p-12 flex flex-col items-center justify-center text-center bg-gray-50/50">
            <span className="text-xs text-gray-400 font-semibold">
              Tidak ada riwayat DTM SDM yang sudah ditandai selesai untuk tahun anggaran{" "}
              {selectedYear}.
            </span>
          </div>
        )}
