...
export default function PeraturanSdm() {
  ...
  const loadKriteria = async () => {
    setIsLoading(true);
    try {
      const data = await apiFetch("/api/sdm/kriteria");
  ...
  const handleSaveKriteria = async (e) => {
    ...
    setIsSaving(true);
    try {
      const updated = await apiFetch("/api/sdm/kriteria", {
        method: "PUT",
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Konfigurasi kriteria default untuk audit Sistem Manajemen Sumber Daya Manusia (SDM).
          </p>
        </div>
      </div>
