...
export default function DtmSdm() {
  ...
  const loadInitialData = async () => {
    setIsLoading(true);
    try {
      const dtmData = await apiFetch("/api/sdm/dtm");
      setDtmList(Array.isArray(dtmData) ? dtmData : []);
    } catch (err) {
      showNotification(err.message || "Gagal mengambil data DTM", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBukaDtm = async (fakultas, dtm) => {
    if (!dtm) return;
    setIsLoading(true);
    setSelectedFakultas(fakultas || dtm.fakultas || null);
    setSelectedDtm(dtm);
    setActiveTab("temuan");
  ...
  const handleExportDocx = async (dtm = selectedDtm) => {
    ...
    try {
      ...
      const unitKerjaName = dtm.fakultas?.namaFakultas || dtm.inputUnitKerja || "Unit_Kerja";
      const yearStr = dtm.tahunAnggaran || "";

      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `DTM_SDM_${unitKerjaName}_${yearStr}.docx`;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {viewMode === "list"
              ? "Daftar monitoring, kriteria audit, dan status persetujuan DTM SDM dari seluruh tahun anggaran."
        ...
        {viewMode !== "list" && (
          <div className="flex items-center gap-2 animate-fadeIn">
            <button
              ...
              disabled={isSaving}
              className="px-3 py-1.5 text-xs font-medium border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 rounded-lg disabled:opacity-50 transition-colors"
            >
              Pratinjau
            </button>
            <button
              ...
              disabled={isSaving}
              className="px-3 py-1.5 text-xs font-medium border border-gray-800 bg-gray-800 text-white rounded-lg hover:bg-gray-700 disabled:opacity-50 transition-colors"
            >
              Ekspor Word
            </button>
            <button
              ...
              disabled={isSaving}
              className={`px-3 py-1.5 text-xs font-medium border rounded-lg disabled:opacity-50 transition-colors ${
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs font-medium border border-gray-300 rounded-lg bg-white text-gray-700 hover:bg-gray-50 transition-colors"
            >
              Kembali
            </button>
          </div>
      ...
      {/* VIEW MODE: HORIZONTAL CARDS LIST */}
      {viewMode === "list" ? (
        isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-gray-500 bg-white border border-gray-200 rounded-xl shadow-xs">
            ...
            <span className="text-sm font-semibold">Memuat berkas DTM SDM...</span>
          </div>
        ) : dtmList.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
              {dtmList.map((dtm) => {
                const isCompleted = dtm.statusDtm === "SUDAH_DITERUSKAN";
                const unitKerjaName = dtm.fakultas?.namaFakultas || dtm.inputUnitKerja || "Unit Kerja Tidak Diketahui";
                const shouldUseCreamCard = !isCompleted;

                return (
                  <div
                    key={dtm.id}
                    onClick={() => handleBukaDtm(dtm.fakultas, dtm)}
          ...
          <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-white border border-gray-250 rounded-xl shadow-xs">
            <FolderKanban className="w-12 h-12 text-gray-300 stroke-1 mb-2" />
            <span className="text-xs font-semibold text-gray-400">Belum ada data DTM SDM yang dibuat.</span>
          </div>
        )
      ) : (
        /* DETAIL WORKSPACE */
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="overflow-x-auto border rounded max-h-[220px] overflow-y-auto">
                            <table className="w-full text-left border-collapse text-[10px]">
                              <thead>
                                <tr className="bg-gray-50 border-b border-gray-200 text-[9px] font-bold text-gray-500">
