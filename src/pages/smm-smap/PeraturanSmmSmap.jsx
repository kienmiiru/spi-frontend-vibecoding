...
export default function PeraturanSmmSmap() {
  ...
  const loadKriteria = async () => {
    setIsLoading(true);
    try {
      const data = await apiFetch("/api/smm-smap/kriteria");
  ...
  const handleSaveKriteria = async (e) => {
    ...
    setIsSaving(true);
    try {
      const updated = await apiFetch("/api/smm-smap/kriteria", {
        method: "PUT",
        body: JSON.stringify({ value: kriteriaText }),
      });
      setKriteriaText(updated.value || "");
      showNotification("Kriteria default audit SMM-SMAP berhasil diperbarui");
    } catch (err) {
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Peraturan & Kriteria SMM-SMAP
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Konfigurasi kriteria default untuk audit Sistem Manajemen Mutu & Sistem Manajemen Anti Penyuapan (SMM-SMAP).
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSaveKriteria} className="space-y-6">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-2">
                Kriteria Default Audit SMM-SMAP
              </label>
              <BulletedTextArea
                value={kriteriaText}
