...
export default function RiwayatDtmBmn() {
  ...
  const handleExportDocx = async (dtm) => {
    ...
    setIsSaving(true);
    try {
      const facultyName = getFacultyName(dtm.fakultasId);
      ...
      a.href = url;
      a.download = `DTM_BMN_${facultyName}_${getTipeBmnLabel(dtm.tipeBmn)}_${selectedYear}.docx`;
  ...
  const handlePreviewDocx = async (dtm) => {
    ...
    try {
      const facultyName = getFacultyName(dtm.fakultasId);
      setPreviewTitle(`Pratinjau DTM BMN - ${facultyName} (${getTipeBmnLabel(dtm.tipeBmn)})`);
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Menampilkan semua DTM yang telah ditandai selesai.
          </p>
        </div>
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-300 text-xs font-semibold text-gray-600">
                  ...
                  <th className="px-6 py-3 border-r border-gray-200">Tipe BMN</th>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          disabled={isSaving}
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 border border-gray-800 bg-gray-800 text-white hover:bg-gray-850/90 rounded text-xs font-medium transition-colors disabled:opacity-50"
          ...
          <div className="border border-gray-150 rounded-lg p-12 flex flex-col items-center justify-center text-center bg-gray-50/50">
            <span className="text-xs text-gray-400 font-semibold">
              Tidak ada riwayat DTM BMN yang sudah ditandai selesai untuk tahun anggaran {selectedYear}.
            </span>
          </div>
        )}
      </div>
