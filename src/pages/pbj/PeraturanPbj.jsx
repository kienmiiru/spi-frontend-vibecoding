...
export default function PeraturanPbj() {
  ...
  const loadKriteria = async () => {
    setIsLoading(true);
    try {
      const data = await apiFetch("/api/pbj/kriteria");
  ...
  const handleSaveKriteria = async (e) => {
    ...
    setIsSaving(true);
    try {
      const updated = await apiFetch("/api/pbj/kriteria", {
        method: "PUT",
      ...
      confirm({
        title: "",
        message: "Kriteria default audit PBJ berhasil diperbarui",
        type: "info"
      });
    } catch (err) {
      showNotification(
      ...
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            Peraturan & Kriteria PBJ
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Konfigurasi kriteria default untuk audit Pengadaan Barang dan Jasa (PBJ).
          </p>
        </div>
      </div>
